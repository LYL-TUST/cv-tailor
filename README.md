...
| 🎯 **JD 匹配诊断** `/ats` | 关键词层 + **语义级逐条职责**双栈分析,缺失关键词按 must/bonus 分级;建议经独立 verifier 校验"相关 / 可执行 / 不诱导编造";**一键按 JD 改写**单条经历(只润色不造事,改写前自动定位简历原文) |
| 🎤 **模拟面试** `/interview` | **简历深挖式出题**(锚定具体经历/量化数字)× 三档面试官风格(标准/温和/压力);**多轮追问链**——补答后面试官沿你的回答继续深挖;限时作答、语音答题/TTS 读题;逐题评估卡:评分+STAR 结构+**回答-简历矛盾点对照**+参考答案+追问应对;整场复盘(追问角度分布/弱题识别)+能力画像雷达+弱题一键收藏再练 |
| 📄 **导出投递** `/download` | 所见即所得 PDF(html2canvas + jsPDF)、真实 DOCX(后端 `docx` 库)、纯文本 |
| 👤 **个人中心** `/me` | 本地历史管理、**收藏夹**(分夹管理面试题,弱题自动归集)、加密备份导出/恢复、**登录后端到端加密云同步**(服务器只存密文) |
...
2. **对抗"AI 简历造假"——真实性护栏贯穿全流程**
   - 导入的 Prompt 温度 0.1 + Zod 强校验,严禁编造原文不存在的信息,缺字段输出核对清单
   - ATS 的每条改进建议都经独立模型校验是否"诚实",一键改写前先定位简历原文依据
   - 模拟面试以"简历深挖"为核心:追问链沿你的回答继续逼问细节、数据来源与个人贡献,并把回答与简历逐点对照,暴露经不起追问的薄弱点——**训练的是"扛住追问的真实经历",不是话术**
   - 前端所有生成按钮旁都标注"需你确认事实"
...
> 环境要求:Node ≥ 22.16(后端 better-sqlite3 依赖较新的 Node ABI);AI 供应商为 OpenAI 兼容协议,DeepSeek 等均可。
...
│   │   ├── favoritesStore.js # 收藏夹(分夹管理 + 弱题归集)
│   │   ├── radarStats.js     # 能力画像雷达(本地聚合)
...
| [演进路线](docs/EVOLUTION_ROADMAP.md) | 版本演进规划(v1 本地优先 → v1.5 云同步 → v2 多人) |
...
- [x] 个人中心:历史 / 收藏夹 / 加密备份 / 一键清空
- [x] Phase 2:可选账号 + 端到端加密云同步(SQLite)
- [x] 模拟面试:简历深挖出题 × 面试官风格 × 多轮追问链 × 整场复盘
- [x] 能力画像雷达 + 弱题收藏再练闭环
- [ ] 求职信(Cover Letter)生成
- [ ] 2 页以上长简历支持
- [ ] 多人协作空间(v2 规划中)
